...
const EditPreferenceModal: React.FC<EditPreferenceModalProps> = ({ open, onClose, pref }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-2 sm:px-4">
      <div className="bg-white rounded-xl shadow-lg p-3 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-md relative">
        <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-700 text-2xl sm:text-3xl" onClick={onClose}>&times;</button>
        <h3 className="text-base sm:text-lg font-bold mb-1 text-[#02457a]">Edit Preference</h3>
        <div className="text-xs sm:text-sm text-[#49708f] font-semibold mb-4 truncate">{pref.productName}</div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:gap-4">
          <div>
            <label className="font-semibold text-sm sm:text-base">Enabled:</label>
            <div className="mt-1 flex items-center gap-2 sm:gap-3">
              <button
                type="button"
                className={`px-2 sm:px-3 py-1 rounded-full font-semibold ${isEnabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
                onClick={() => setIsEnabled(true)}
              >Yes</button>
              <button
                type="button"
                className={`px-2 sm:px-3 py-1 rounded-full font-semibold ${!isEnabled ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}
                onClick={() => setIsEnabled(false)}
              >No</button>
            </div>
          </div>
          <div>
            <label className="font-semibold text-sm sm:text-base">Notification Types:</label>
            ...
            <div className="flex gap-1 sm:gap-2 mt-2 items-center">
              ...
              <button type="button" className="bg-blue-500 text-white px-2 sm:px-3 py-1 rounded text-xs sm:text-sm" onClick={handleAddType} disabled={!selectedType}>Add</button>
            </div>
          </div>
          <button type="submit" className="bg-[#02457a] text-white px-3 sm:px-4 py-2 rounded font-semibold mt-2 disabled:opacity-60 text-sm sm:text-base" disabled={loading}>
            {loading ? 'Saving...' : 'Save'}
          </button>
        </form>
      </div>
